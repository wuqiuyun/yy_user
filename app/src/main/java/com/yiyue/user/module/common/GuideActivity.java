package com.yiyue.user.module.common;
...
import com.yiyue.user.model.local.preferences.SystemConfigSharedPreferences;
import com.yiyue.user.module.main.MainActivity;
import com.yiyue.user.widget.ImageViewPager;
...
public class GuideActivity extends BaseAppCompatActivity {
    ...
    private void addGuideBg(int bg, boolean isWait) {
        ...
        if (isWait) {
            try {
                Glide.with(getApplicationContext()).load("android.resource://com.yiyue.user/drawable/" + bg).into(imageView);
            } catch (Exception e) {
                e.printStackTrace();
