package com.yiyue.user.util.easyutils;
...
import com.yiyue.user.util.easyutils.emlisenter.MyCallBackImpl;
import com.yiyue.user.util.easyutils.emlisenter.MyEMMessageListener;
...
public class EMInterfaceImpl implements EMInterface {
    ...
    @Override
    public void deleteConversation(String imId) {
        //删除和某个user的整个的聊天记录（包括本地）
        EMClient.getInstance().chatManager().deleteConversation(imId, true);
    }
     ...
     */
    @Override
    public EMMessage sendOrderAddMoneyMes(SelfDefinedInfoBean chatNoFriendBean, String mChatId) {
        EMMessage message = EasyUtil.getEmManager().createTxtSendMessage(chatNoFriendBean.getContent(), mChatId);
        message.setChatType(EMMessage.ChatType.Chat);
        message.setAttribute(Constants.IM_MESSAGE_ORDER_ADDMONEY_KEY, new Gson().toJson(chatNoFriendBean));//染发 加价 ！之类！自定义消息类型  用这个key传值/ios端也能识别
        // 调用发送消息的方法
        sendMessage(message);
        return message;
    }
    ...
    /**
     * 同意或者拒绝 加价 消息发送方法
     * 需要传 订单发起者（美发师）的环信id，用户id，昵称，头像路径, 服务类型，加价金额,订单详情id
     *
     * @param context
     * @param mChatId
     * @param userid
     ...
     * @param msgStatus   1为发送人发起加价状态 2为已同意  3.为已拒绝
     */
    @Override
    public void dealWithAddMoneyMes(Context context, String mChatId, String userid, String name, String path, String serviceName, Double money, String detailId, int msgStatus) {
        ...
        if (!TextUtils.isEmpty(myUserId)) {
            ...
            selfDefinedInfoBean.setToUserId(Long.parseLong(myUserId));
        }

        selfDefinedInfoBean.setToImusername(AccountManager.getInstance().getAccount().getImusername());
        selfDefinedInfoBean.setToNickname(TextUtils.isEmpty(AccountManager.getInstance().getAccount().getNickname()) ? AccountManager.getInstance().getUsername() : AccountManager.getInstance().getAccount().getNickname());
        selfDefinedInfoBean.setToPath(AccountManager.getInstance().getAccount().getHeadImg());
